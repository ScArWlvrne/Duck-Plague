// error.cpp ? error reporting and failsafe logging; feeds controller so it can transition to Restore.
#include <string>
#include <fstream>
...
void log_error(const std::string& source, const std::string& message, const std::string& logPath) {
    ...
    if (log) {
        log << "------------------------------" << std::endl;
        log << "ERROR [" << source << "] " << message << std::endl;
        log << "------------------------------" << std::endl;
    }
}
...
UiRequest error_display(const Context& /*ctx*/) {
    std::string body = s_lastErrorReason.empty()
        ? "An error occurred."
        : s_lastErrorReason;
